...
// .wrangler/tmp/bundle-EA3ByM/strip-cf-connecting-ip-header.js
...
var JWT_SECRET = "your-secret-key-change-in-production-this-should-be-at-least-32-chars-long";
var SECRET_KEY = new TextEncoder().encode(JWT_SECRET);
var DEMO_USER = {
  id: "1",
...
app.post("/api/auth/login", async (c) => {
  try {
    ...
    if (username === DEMO_USER.email && password === "demo1234") {
      try {
        const expirationTime = Math.floor(Date.now() / 1e3) + 7 * 24 * 60 * 60;
        const token = await new SignJWT({ sub: DEMO_USER.id }).setProtectedHeader({ alg: "HS256" }).setExpirationTime(expirationTime).sign(SECRET_KEY);
...
app.get("/api/sales/analytics/trends", (c) => {
  ...
  for (let i = 0; i < parseInt(days); i++) {
    ...
    data.unshift({
      ...
      sales: Math.floor(Math.random() * 5e3) + 1e3,
...
app.get("/api/sales/analytics/competitor-pricing", (c) => {
  return c.json([
    { competitor: "Competitor A", price: 45.99, your_price: 49.99, difference: 4 },
    { competitor: "Competitor B", price: 52.99, your_price: 49.99, difference: -3 }
  ]);
});
...
// .wrangler/tmp/bundle-EA3ByM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-EA3ByM/middleware-loader.entry.ts
